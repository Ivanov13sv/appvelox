...
export const RegSecondStep = () => {
    ...
    const registrationAddress = useInput(user?.registrationAddress, {
        isEmpty: true,
    });
    const gender = useInput(user?.gender || 'male');
    const date = useInput(user?.dOb ? new Date(user.dOb) : null, {
        isEmpty: true,
    });
    const residentialAddress = useInput(user?.residentialAddress, {
        isEmpty: true,
    });
